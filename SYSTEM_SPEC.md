...
- The automation scheduler (see `intuition.scheduler.core` + `dev.scheduler`) polls `missions/list-ready-missions`, orders candidates by §3.6 priority, and launches governed agent sessions via Codex/Agent Gateway commands. Every successfully claimed mission writes `scheduler-run.edn` under `missions/logs/<mission-id>/scheduler/` with the selected mission summary, timestamps, and command payload so §§3.3–3.6, §5.1, §5.3, §6.2, and §9 have an audit trail. If the agent command fails or the mission remains `:mission.status/ready`, the scheduler also records `scheduler-failure.edn` with the exit code + gateway status so stewards can escalate immediately.
